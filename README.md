...
On the 43-puzzle dataset this was first measured against:
...
`utils/calibrate.js`'s search space).

The existing rule heuristic already reaches very high tier accuracy on the
local dataset, so there is little headroom to demonstrate further gains from
this feature alone. A joint sweep with the calibrator picks
...
**Accuracy: 49/50 (98%)** across puzzles with community stats.
<!-- BENCHMARK:END -->
